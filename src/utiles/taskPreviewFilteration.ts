...
export const filterTasks = (
  ...
  if (selectedTimeFrame.length > 0) {
    ...
    if (selectedTimeFrame.includes('today')) {
      filteredTasks = filteredTasks.filter(task => isToday(new Date(task.createdAt)))
    } else if (selectedTimeFrame.includes('yesterday')) {
      filteredTasks = filteredTasks.filter(task => isYesterday(new Date(task.createdAt)))
  ...
  if (selectedLabels.length > 0) {
    filteredTasks = filteredTasks.filter(task => task.label.some(l => selectedLabels.includes(l)))
  }
