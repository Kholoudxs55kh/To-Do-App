...
} from "../middlewares/task.validate";
...
export default class TaskController {
  ...
  static softDeleteAllActiveTasks = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    const taskDao = new TaskDao();
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        res
          .status(500)
          .json({ error: "An unknown error occurred while soft deleting tasks" });
      }
    }
  };

  static softDeleteAllDoneTasks = async (
    req: Request,
    res: Response
  ): Promise<void> => {
    const taskDao = new TaskDao();
